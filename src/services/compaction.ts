...
export function createCompactionHook(
  ...
  async function fetchProjectMemoriesForCompaction(): Promise<string[]> {
    try {
      const result = await vikingMemoryClient.listMemories(tags.project, CONFIG.maxProjectMemories);
  ...
  async function checkAndTriggerCompaction(sessionID: string, lastAssistant: MessageInfo): Promise<void> {
    ...
    await ctx.client.tui.showToast({
      body: {
        ...
        message: `Context at ${(usageRatio * 100).toFixed(0)}% - compacting with VikingMemory context...`,
        variant: "warning",
        duration: 3000,
      },
    }).catch(() => {});
    ...
    try {
      ...
      await ctx.client.tui.showToast({
        body: {
          ...
          message: "Session compacted with VikingMemory context. Resuming...",
          variant: "success",
          duration: 2000,
        },
      }).catch(() => {});
